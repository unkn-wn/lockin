...
export function FaceTrackedVideo({ className, onVideoReady, width, height }: FaceTrackedVideoProps) {
    ...
    return (
        <div className={`relative overflow-hidden rounded-lg border border-neutral-300 bg-black align-top shadow h-auto w-full ${className}`}>
            <video className="-scale-x-[1]" ref={videoRef} autoPlay playsInline></video>
